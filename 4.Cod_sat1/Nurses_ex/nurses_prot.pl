...
% nurseIDandBlocking( nurse15, 14, 22 ).
% nurseIDandBlocking( nurse16, 14, 22 ).
% nurseIDandBlocking( nurse17, 14, 22 ).
% nurseIDandBlocking( nurse18, 14, 22 ).
% nurseIDandBlocking( nurse19, 14, 22 ).
% nurseIDandBlocking( nurse20, 14, 22 ).
% nurseIDandBlocking( nurse21, 14, 22 ).
% nurseIDandBlocking( nurse22, 14, 22 ).
% nurseIDandBlocking( nurse23, 14, 22 ).
% nurseIDandBlocking( nurse24, 14, 22 ).
% nurseIDandBlocking( nurse25, 14, 22 ).
...
noWorkInRange:- nurseIDandBlocking(N,HS,HE), hourInRange(HS,HE,H), negate(Lit,worksNH-N-H), writeClause([Lit]), fail.
noWorkInRange.

nurseType:- nurse(N), findall(nurseType-N-Type, type(Type), Lits), exactly(1, Lits), fail.
nurseType.  %% Modificar con expressOr

startsWorkOnly1:- nurse(N), findall( startsNH-N-H, hour(H) , Lits), exactly(1,Lits), fail.
startsWorkOnly1.

nurseMin:- hour(H), needed(H,M),findall( worksNH-N-H, nurse(N) , Lits), atLeast(M,Lits), fail.
nurseMin.

nurseWork:- 
    nurse(N), hour(H), type(T),
    findall(worksNH-N-HW, workingHourForTypeAndStartH(T,H,HW), Lits1),
    findall(worksNH-N-HW2, hourInRange(0,24,HW2), Lits2),
    subtract(Lits2,Lits1,Lits),
    negateAll(Lits,Nlits),
    writeIfThen(startsNH-N-H, nurseType-N-T, Lits1),
    writeIfThen(startsNH-N-H, nurseType-N-T, Nlits),  
    fail.
nurseWork.

writeIfThen(S, T, [C|L]):- 
    negate(NS,S), negate(NT,T), 
    writeClause([NS,NT,C]), 
    writeIfThen(S,T, L).
writeIfThen(_,_,[]).

% nurseWork:- nurse(N), type(T) , hour(H),
%   findall(worksNH-N-HW, workingHourForTypeAndStartH(T,H,HW), Lits),
%   expressOr(worksNH-N-HW,Lits),  fail.
% nurseWork.
...
% nurseMin2:-
%     hour(H), type(T),
%     workingHourForTypeAndStartH(T,H,HW),needed(HW,M), findall(worksNH-N-HW,nurse(N) , Lits),exactly(M,Lits),
%     fail.
% nurseMin2.
% 
% allNurses:- nurse(N), findall(worksNH-N-H, hour(H),Lits), exactly(6,Lits), fail.
% allNurses.


% nurseWork:-
%     nurse(N), hour(H),
%    findall(worksNH-N-HW,workingHourForTypeAndStartH(1,H,HW),Lits1),
%    findall(worksNH-N-HW2, hourInRange(0,24,HW2), Lits2),
%    subtract(Lits2,Lits1,Lits),
%    negateAll(Lits,Nlits),
%    expressOr(startsNH-N-H, Nlits),
%    fail.
% nurseWork.


%%% OR entre hora de inici work y hora final work
% nurseWork:-
%     nurse(N), hour(H),
%     findall(worksNH-N-HW, workingHourForTypeAndStartH(1,H,HW), Lits),
%     expressOr(startsNH-N-H, Lits),
%     fail.
% nurseWork.
...
%    nurse(N),hour(H),
%    findall(worksNH-N-HW, workingHourForTypeAndStartH(1,H,HW), Lits), 
%    writeClausesWork(startsNH-N-H, Lits), fail.
% nurseWork.
%  
% writeClausesWork(S, [C|L]):- expressOr(S, [C]), writeClausesWork(S, L).
% writeClausesWork(_,[]).


%typeWork:- nurse(N), findall(nurseType-N-Type, type(T), Lits), exactly(1,Lits), fail.
%typeWork.
