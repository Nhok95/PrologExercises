...
%   2. noTask-G-H means:  "gangster G do nothing at hour H"

writeClauses(K):- 
    initClauseGeneration,
    eachHourEachGangsterAtMostOneTask,
    neededTask,
    noAnotherTaskOnConsecutive,
    %consecutiveHoursGangster(K),
...
neededTask:- hour(H), task(T), needed(T,H,N), findall(does-G-T-H, gangster(G), Lits), atLeast(N, Lits), fail.
neededTask.

consecutiveHoursGangster(K):- gangster(G), task(T), findall(does-G-T-H, hour(H), Lits), atLeast(K,Lits),  fail.
consecutiveHoursGangster(_).
% 
noAnotherTaskOnConsecutive:- gangster(G), task(T), 
noAnotherTaskOnConsecutive.

maxConsecutiveHours(M,K):- length(M,N), K is N.

%gangsterNotAvailable:- gangster(G), blocked(G,H), writeClauses([\+does-G-]), fail.
%gangsterNotAvailable.
...
% Express that Var implies Lit:
expressIfThen(Var, Lits):- member(Lit,Lits), writeClause([\+Var,Lit]),fail. 
expressIfThen(_,_).
