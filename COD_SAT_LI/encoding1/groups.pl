...
writeClauses:-
    eachSexactlyOneG,  %writeClause([]),
    noSincompat, %writeClause([]),
    atLeastMinG, %writeClause([]),
    atMostMaxG, %writeClause([]),
    %exactlyKGroups, innecesario, group(G) se encarga de que solo existan N grupos
    true.
...
noSincompat:-
    ...
    %exactly(1,[sg-S1-G,sg-S2-G]),
    fail.
noSincompat.

atLeastMinG:-
    group(G), minGroup(Min),
    findall(sg-S1-G, student(S1), Lits),
    atLeast(Min,Lits),
    fail.
atLeastMinG.

atMostMaxG:-
    group(G), maxGroup(Max),
    findall(sg-S1-G, student(S1), Lits),
    atMost(Max,Lits),
    fail.
atMostMaxG.
