...
cities(List):-  findall(C, city(C,_), List).
getStations(C,S):-  city(C,Stations), member(S, Stations).  
getImportants(C):- importantCities(List), member(C, List).

allStations(S):- numStations(N), between(1,N,S).

allStationsCost(St,C):- C is 1, between(1,5,St).
allStationsCost(St,C):- C is 1, between(11,20,St).
allStationsCost(St,C):- C is 2, between(1,5,St).


maxCost(S):-
    findall(1, allStationsCost(_,_), Lits),
    length(Lits,S).
...
writeClauses(K):-
    usedStationMeans,
    importantCitiesMustBeCoveredForTwoStations,
    notImportantCitiesAtLeastOneStation,
    atMostKCostStations(K),
    true,!.
    
usedStationMeans:-
    allStationsCost(S,C),
    writeClause([\+usedStations-S, cost-S-C]),
    fail.
usedStationMeans.

importantCitiesMustBeCoveredForTwoStations:-
    getImportants(C),
    findall(usedStations-S,getStations(C,S),Lits),
    atLeast(2,Lits),
    fail.
importantCitiesMustBeCoveredForTwoStations.
...
atMostKCostStations(K):-
    findall(cost-S-C, allStationsCost(S,C), Lits),
    atMost(K,Lits).
atMostKStations(_).
...
cost(M,Cost):-
    ...
    between(0,N,R),
    Cost is N-R,
    findall(1, member(cost-S-_, M),L),
    length(L,Cost).


%Show the solution:	
displaySol(M):-
    findall(S, member(usedStations-S, M), Lits),
    sort(Lits,OLits),
    writeSol(OLits).
displaySol(_).
